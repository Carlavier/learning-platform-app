...
export function UserDashboard() {
  ...
  const averageProgress = progress.length > 0 ? Math.round(totalProgress / progress.filter(p => p.userId === currentUser.id).length) : 0;
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-gray-600 shrink-0">{prog.progress}%</span>
                        </div>
                      </div>
                    </div>
                  );
            ...
            {recentLessons.length > 0 ? (
              <div className="space-y-4">
                {recentLessons.map(lesson => {
                  const prog = progress.find(p => p.lessonId === lesson.id && p.userId === currentUser.id);
