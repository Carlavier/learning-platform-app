...
export const lessonsApi = {
  ...
  createLesson: async (
    data: Omit<Lesson, 'id' | 'createdAt' | 'createdBy'>
  ): Promise<Lesson> => {
    const token = localStorage.getItem(STORAGE_KEYS.ACCESS_TOKEN);
    if (!token) {
...
export const progressApi = {
  ...
  getUserProgress: async (userId: string): Promise<Progress[]> => {
    const response = await fetch(
      `${API_CONFIG.baseURL}/progress?user_id=eq.${userId}`,
      {
        headers: getAuthHeaders(),
      }
    );
  ...
  getLessonProgress: async (
    ...
    const response = await fetch(
      `${API_CONFIG.baseURL}/progress?user_id=eq.${userId}&lesson_id=eq.${lessonId}`,
      {
        headers: getAuthHeaders(),
      }
    );
  ...
  updateProgress: async (
    ...
    // PostgREST doesn't return data on upsert by default, so fetch it
    // @ts-expect-error - TypeScript doesn't track that we check for undefined below
  ...
  markLessonAccessed: async (userId: string, lessonId: string | number): Promise<Progress> => {
    // Get current progress or create new one
    // @ts-expect-error - TypeScript doesn't track that optional chaining handles undefined
